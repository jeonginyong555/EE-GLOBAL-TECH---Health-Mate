...
const STUDENT_ID = "2025730079";
const STUDENT_NAME = "lee sejeong";
...
function showScreen(name) {
  ...
  // 혹시 home-screen이 HTML에 남아있으면 항상 숨김 처리
...
// home 관련 버튼이 HTML에 남아있어도 숨김 처리
const goModeBtn = document.getElementById("goModeBtn");
const showGuideBtn = document.getElementById("showGuideBtn");
const goHomeBtn = document.getElementById("goHomeBtn");

if (goModeBtn) goModeBtn.style.display = "none";
if (showGuideBtn) showGuideBtn.style.display = "none";
...
updateResultCards();
setRunButtonState();
setAnalysisStatus("Ready (웹캠/비디오 선택 → 준비하기 → 시작)");
logInfo("(no data)");

// 시작하자마자 입력 모드 선택 화면으로 진입
showScreen("mode");