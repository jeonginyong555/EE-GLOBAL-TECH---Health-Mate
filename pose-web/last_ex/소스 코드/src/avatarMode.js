...
export async function createMode({ dom, shared }) {
  ...
  async function inferPose() {
    if (poseBusy || !poseLandmarker || !avatarManager || !dom.video.videoWidth) return;
    poseBusy = true;
    try {
      const ts = performance.now();
      const result = poseLandmarker.detectForVideo(dom.video, ts);
      if (result?.worldLandmarks?.[0]) {
        avatarManager.update({ poseLandmarks: result.worldLandmarks[0] });
      }
    } finally {
      poseBusy = false;
    }
    ...
    return new Promise((resolve, reject) => {
      loader.load(
        ...
        (gltf) => {
          ...
          avatarManager = new MediapipeAvatarManager();
          avatarManager.bindAvatar(avatarScene, "Mixamo");
          avatarManager.setUseKalmanFilter(true);
          avatarManager.setSlerpRatio(0.5);
          avatarManager.setUseHand(false);
