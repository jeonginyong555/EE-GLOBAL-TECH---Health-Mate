...
class MediapipePoseCalculator {
    constructor() {
        ...
        this.updatePose = (poseData, dtype) => {
            ...
            const fixedPoseData = poseData.map(d => ({
                ...d,
                x: -d.x,
                visibility: (d.visibility ?? 0) < 0.2 ? 0 : (d.visibility ?? 1),
            }));

            this.poseLandmarkManager.setFromArray(fixedPoseData, dtype);
        ...
        this.getWorldRootPos = () => {
            ...
            const worldRootPos = (new THREE.Vector3())
                .add(leftHip.worldPosition)
                .add(rightHip.worldPosition)
                .multiplyScalar(0.5);
        ...
        this.updateHand = (handData, dtype, isLeft) => {
            ...
            const fixedHandData = handData.map(d => ({
                ...d,
                x: -d.x,
            }));
            ...
            handLandmarkManager.setFromArray(fixedHandData, dtype);
        ...
        let setPoseBoneCalculator = () => {
            ...
            Head.update = function () {
                ...
                let lm_right_ear = lm_head && lm_head.getChild(7);
        ...
        let setHandBoneCalculator = (isLeft) => {
            ...
            ThumbMetacarpal.update =
            IndexMetacarpal.update =
            MiddleMetacarpal.update =
            RingMetacarpal.update =
            ...
            ThumbProximal.update =
            ThumbDistal.update =
            IndexProximal.update =
            IndexIntermediate.update =
            IndexDistal.update =
            MiddleProximal.update =
            MiddleIntermediate.update =
            MiddleDistal.update =
            RingProximal.update =
            RingIntermediate.update =
            RingDistal.update =
            PinkyProximal.update =
            PinkyIntermediate.update =
            PinkyDistal.update = function () {
                let thisYDirection = this.yDirection;

                let baseXDirection = null;
                let baseZDirection = null;

                if (this.name.includes('thumb')) {
                    baseXDirection = ThumbMetacarpal.xDirection;
                    baseZDirection = ThumbMetacarpal.zDirection;
                } else if (this.name.includes('index')) {
                    baseXDirection = IndexMetacarpal.xDirection;
                    baseZDirection = IndexMetacarpal.zDirection;
                } else if (this.name.includes('middle')) {
                    baseXDirection = MiddleMetacarpal.xDirection;
                    baseZDirection = MiddleMetacarpal.zDirection;
                } else if (this.name.includes('ring')) {
                    baseXDirection = RingMetacarpal.xDirection;
                    baseZDirection = RingMetacarpal.zDirection;
                } else if (this.name.includes('pinky')) {
                    baseXDirection = PinkyMetacarpal.xDirection;
                    baseZDirection = PinkyMetacarpal.zDirection;
                }

                if (baseXDirection && baseZDirection && thisYDirection) {
                    let angle = thisYDirection.angleTo(baseXDirection);

                    if (angle > Math.PI / 4 && angle < 3 * Math.PI / 4) {
                        zDirection.crossVectors(baseXDirection, thisYDirection);
                        zDirection.negate();
                    } else {
                        zDirection.crossVectors(baseZDirection, thisYDirection);
